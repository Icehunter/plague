...
3. **Record the decision and its trail, not only the outcome.** A decision without its measurement
   gets argued again in three weeks. Close-out records and investigation notes live in `docs/local/`,
   on disk and kept out of git on purpose; only the permanent documents below are tracked.
...
5. **`docs/KNOWN-ISSUES.md` is one line per open defect**, pointing at the code. An issue states
   what is seen and what would resolve it. The long write-up behind it belongs in `docs/local/`, not
