...
2. **Block textures and captures are not committed.** They are not the pack's to redistribute, and
   they are large. Both resolve at run time: textures from the installed resource pack, captures
   from `$PLAGUE_CAPTURES` (default `~/plague-captures`). See `tools/captures_dir.py`. A tool that
   needs one skips its comparison and says so when it is absent; it never crashes and never silently
   passes.
...
An `ASSETS.md` row carries more than a licence. Write down what a later reader would need and cannot
work out from the file itself: how it was produced, what a regenerating script is called, which
...
- Hand-editing a generated texture instead of the generator that produced it. The next rerun
  silently reverts the edit and the provenance claim becomes false.
