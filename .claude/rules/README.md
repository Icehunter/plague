...
glob. They are in context because `AGENTS.md` links them from its "Modular Rules" table, so they are
all always on or all always off. The key records the intended scope for a reader; it scopes nothing.
Keep that table and the list below in sync, and do not rely on a rule "only applying" to its glob.
...
1. **Only document what is observably true in this tree.** Every count, path and constant in these
   files came from reading the code. Verify a claim before adding it.
