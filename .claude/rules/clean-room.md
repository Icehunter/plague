...
**Copyright protects expression, not ideas.** Plague ships under MIT, which is a *grant*: you cannot
grant what you do not own. The bar is not "does it look different", it is "did this come out of the
writer's own reasoning".

Reading other shaderpacks is how you learn what is possible, and it is allowed. The reading and the
writing have to be separated, and that separation is what this file is.
...
- **Authored constants**: numbers that encode taste rather than physics. A colour table, a damping
...
- **A deviation from published physics.** The sharpest tell there is. Where a scattering or
  ...
  *choice*, not a measurement, and there is no innocent way to arrive at it. Take the number
...
The reader's output is the airlock. A fact that cannot survive being written in prose is expression,
...
- **Physically derived**: keep it, but cite the paper, measurement or standard in the comment. The
...
expression that evaluates to the same thing, hides it from a text search while keeping exactly the
thing that mattered. Derive it forwards from a stated criterion, and let it land where it lands.
...
capture` says more to the next reader than a pointer at somebody else's variable.

**One hard sequencing rule.** A comment describing where code came from is removed only in the same
change that replaces the code beneath it, never ahead of it. Removing the note while keeping the
code changes nothing about the code and looks like concealment.
...
interface this pack implements. Naming an interface to speak to it has never been infringement.
...
gate. It catches a project *name*; it cannot catch a euphemism or a copied statement order, so it is
a backstop, not the protocol.
...
Generate authored tables with a committed script rather than pasting numbers: start positions,
palettes, sample kernels. The script is the record that the table is yours, and it re-runs.
...
expression. Measure the sub-field a viewer tracks, not just an aggregate: an aggregate statistic can
be satisfied by the wrong term while the thing you care about does not move.
