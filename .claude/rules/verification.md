...
There is no CI and no test framework in this repository. Checks live offline under `tools/`: one
compile check, per-subsystem offline models, parity fits against measured fixtures, table
generators, and a renderer for looking at a look change.

**Most of it is local only and not tracked.** Only the table generators ship, because shaders cite
them as the source of their constant tables. The rest sits on disk, works as described below, and is
no use to someone installing a shaderpack. A path named here that is not in the repository is
missing for that reason.

## Rules

1. **`tools/check_shaders.sh` is the compile gate.** It flattens `#moj_import`, compiles every pass
   under the arms listed in its own `variants_for()`, then runs four water verifiers: 46 source
   files, 153 compile invocations and 565 checks green as of 2026-08-23.
   `tools/pre-commit` runs it, and the attribution lint first, whenever a commit touches `shaders/`,
   `graph.toml`, `screens.toml` or the notices files. Install with `tools/install-hooks.sh`; it
   symlinks the tracked hook, so updates take effect without reinstalling.
2. **A default arm compiling is not a check.** An option that is default-off leaves its whole
   feature out of the build, and the script reports `ok` on code that never met a compiler. This is
   the most repeated failure in the repository's history; add the variant in the same change.
...
The compile gate needs `glslangValidator` (`brew install glslang`); the pre-commit hook skips
cleanly when it is absent, so a machine without Vulkan tooling can still commit.

**A verifier parses the live GLSL rather than restating its constants.** `verify_water_waves.py`
opens `shaders/include/water_waves.glsl` and `shaders/post/water_composite.fsh` and reads the
numbers out of them, so the model cannot drift away from the shader. Follow that: `check(name,
condition, detail)` printing `ok`/`FAIL`, a failure list, and a non-zero exit.
...
tuned. Render the user's own scene at the user's own option values, then pick the constant off the
picture.
...
- [ ] Look changes rendered, the user's own scene at the user's own option values, and looked at
