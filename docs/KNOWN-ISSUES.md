...
- **A water surface draws over a cloud in front of it.** Looking down from above the deck, ground
  ...
  this needs. Reordering is ruled out: `ssr_trace_water` reads `sceneHdr` and its sky probe samples
  the on-screen sky pixel to stay consistent with the direct view, so clouds have to be in
  `sceneHdr` before SSR, and SSR runs before water. Clouds cannot be both before SSR and after water
  with one composite. The fix is for `water_composite` to scale its own alpha by how much light gets
  through the cloud where `cloudFrontDistance` is nearer than the water surface, which needs the
  cloud distance in that pass: its targets are tiered on `CLOUD_QUALITY` and the pass is not, so it
  needs either tier variants or a single ungated distance target.
