...
uniform sampler2D u_HeatBlurH; // heatBlurH: the horizontal pass's own output
...
void main() {
    if (u_WorldBounds.w != 2.0) {
        fragColor = texture(u_HeatBlurH, texCoord);
        return;
    }

    float radiusPx = PLAGUE_HEAT_BLUR_RADIUS_PX * (float(textureSize(u_HeatBlurH, 0).y) / 1080.0);
    float sigma = max(radiusPx, 1.0) / 2.0;
    float texelY = 1.0 / float(textureSize(u_HeatBlurH, 0).y);
    ...
    for (int i = -TAPS; i <= TAPS; i++) {
        ...
        vec3 tap = texture(u_HeatBlurH, tapUv).rgb;
