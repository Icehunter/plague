...
// directly, so the two convolutions compose into one genuine 2D Gaussian. u_Depth/u_WaterDepth stay
...
uniform sampler2D u_UnderwaterBlurH; // underwaterBlurH: the horizontal pass's own output
uniform sampler2D u_Depth; // builtin.depth, full resolution; ordinal parity only, unused (see header)
uniform sampler2D u_WaterDepth; // builtin.waterDepth, full resolution; ordinal parity only, unused (see header)
...
void main() {
    ...
    if (u_WaterState.x <= 0.5 || u_UwBlurRadius <= 0.0) {
        fragColor = texture(u_UnderwaterBlurH, texCoord);
        return;
    }

    float radiusPx = u_UwBlurRadius * (float(textureSize(u_UnderwaterBlurH, 0).y) / 1080.0);
    float sigma = max(radiusPx, 1.0) / 2.0;
    float texelY = 1.0 / float(textureSize(u_UnderwaterBlurH, 0).y);
    ...
    for (int i = -TAPS; i <= TAPS; i++) {
        ...
        vec3 tap = texture(u_UnderwaterBlurH, tapUv).rgb;
