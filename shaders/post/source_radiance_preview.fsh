...
uniform sampler2DArray u_ConsolidatedGbuf; // consolidatedGbuf: existing RGB, canonical RGB, then surface class
uniform sampler2D u_Depth;      // opaque depth; water/forward surfaces are outside this preview
in vec2 texCoord;
out vec4 fragColor;
...
void main() {
    ...
    if (texelFetch(u_Depth, pixel, 0).r <= 0.0) return; // reversed-Z clear value is sky
    // gAo.a's ABI uses quarter steps: 4 = solid terrain, 2 = cutout terrain, others are excluded.
    int surfaceClass = int(round(texelFetch(u_ConsolidatedGbuf, ivec3(pixel, 2), 0).a * 4.0));
    ...
    fragColor.rgb = texelFetch(u_ConsolidatedGbuf, ivec3(pixel, sourceLayer), 0).rgb;
#endif
}
