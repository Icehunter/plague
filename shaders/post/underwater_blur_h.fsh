...
// across silhouettes rather than preserving edges — that is the brief, not a bug. u_Depth/u_WaterDepth
...
void main() {
    ...
    float radiusPx = u_UwBlurRadius * (float(textureSize(u_SceneHdrRefracted, 0).y) / 1080.0);
    ...
    fragColor = texture(u_SceneHdrRefracted, texCoord);
#endif
}
