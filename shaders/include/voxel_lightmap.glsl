...
uniform usamplerBuffer u_VoxelLightmap; // engine light bytes, independent of the material palette
...
bool plagueVoxelLightAt(vec3 point, out vec2 light) {
    ...
    if (d <= 0 || d > 33 || textureSize(u_VoxelLightmap) != d*d*d*1024
            || textureSize(u_VoxelBrickSummary) != d*d*d) return false;
    ...
    uint value = (texelFetch(u_VoxelLightmap,slot*1024+(index>>2)).r >> uint((index&3)*8)) & 255u;
